      ...
      <main className={`writing-words-main ${sidebarOpen ? 'with-sidebar' : 'without-sidebar'}`}>
        <div className="writing-words-container">
          ...
          <div className="writing-words-header">
            <h1 className="writing-words-title">Пропись слов</h1>
            <p className="writing-words-subtitle">Выберите букву для просмотра примеров написания слов</p>
          </div>
          ...
          <div className="writing-words-letters">
            {letters.map((letter, index) => (
              <button
                key={index}
                className={`writing-words-letter-btn ${selectedLetter === letter.char ? 'active' : ''}`}
