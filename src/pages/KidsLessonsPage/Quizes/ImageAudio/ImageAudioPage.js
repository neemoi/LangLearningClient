...
import QuizHeader from '../../../KidsLessonsPage/Quizes/ImageAudio/QuizHeader';
import QuizStartScreen from '../../../KidsLessonsPage/Quizes/ImageAudio/QuizStartScreen';
import QuizQuestionSection from '../../../KidsLessonsPage/Quizes/ImageAudio/QuizQuestionSection';
import QuizResultsSection from '../../../KidsLessonsPage/Quizes/ImageAudio/QuizResultsSection';
import QuizScoreDisplay from '../../../KidsLessonsPage/Quizes/ImageAudio/QuizScoreDisplay';
import QuizOtherTests from '../../../KidsLessonsPage/Quizes/ImageAudio/QuizOtherTests';
import Navigation from '../../../../components/layout/Navigation/Navigation';
import Sidebar from '../../../../components/layout/Sidebar/Sidebar';
import '../../css/ImageAudio.css';
...
const QuizImagePage = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [questionsRes, wordsRes, lessonRes, progressRes] = await Promise.all([
          fetch(`${API_CONFIG.BASE_URL}/api/KidQuizQuestions/byLesson/${lessonId}?quizTypeId=3`),
        ...
        const filteredQuestions = questionsData.filter(q => q.quizTypeId === 3);
        if (filteredQuestions.length === 0) {
          throw new Error('Для этого урока нет вопросов типа "Картинки+Аудио"');
        }
  ...
  const generateOptions = (question, allWords) => {
    ...
    const wrongWords = allWords
      .filter(word => word.id !== correctWord?.id && word.imageUrl)
    ...
    try {
      ...
      const response = await fetch(`${API_CONFIG.BASE_URL}/api/kid-progress/lesson`, {
        ...
        body: JSON.stringify({
          ...
          learnedWords: correctCount,
          testResults: [{
            testType: 'image_audio_choice',
            score: percentage,
      ...
      if (existingProgress) {
        const response = await fetch(`${API_CONFIG.BASE_URL}/api/kid-progress/word`, {
          ...
          body: JSON.stringify({
            ...
            lessonId: parseInt(lessonId),
            wordId: word.id,
            questionType: 'image_audio_choice',
            isCorrect: isCorrect
          })
        });
  ...
  return (
    <div className="ia-app">
      ...
      <div className="ia-content">
        ...
        <main className={`ia-main-content ${sidebarOpen ? 'sidebar-open' : 'sidebar-closed'}`}>
          <QuizHeader lessonTitle={lessonTitle} lessonId={lessonId} />
          
          <div className="ia-container">
            {!testStarted ? (
              <QuizStartScreen onStart={startTest} />
            ) : !completed ? (
              <>
                <QuizQuestionSection 
                  options={options}
          ...
          <QuizOtherTests lessonId={lessonId} />
        </main>
      </div>
    </div>
  );
